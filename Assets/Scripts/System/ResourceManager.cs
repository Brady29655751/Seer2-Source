...
public class ResourceManager : Singleton<ResourceManager>
{
    ...
    private async void LoadMapResources(Map map, Action<Map> onSuccess = null) {
        int resId = (map.resId == 0) ? map.id : map.resId;
        Sprite bg = await Addressables.LoadAssetAsync<Sprite>(mapPath + "bg/" + resId).Task;
        Sprite pathSprite = await Addressables.LoadAssetAsync<Sprite>(mapPath + "path/" + Mathf.Abs(resId)).Task;
        AudioClip bgm = await Addressables.LoadAssetAsync<AudioClip>(BGMPath + map.music.category + "/" + map.music.bgm).Task;
        AudioClip fx = string.IsNullOrEmpty(map.music.fx) ? null : await Addressables.LoadAssetAsync<AudioClip>(map.music.fx).Task;
    ...
    private IEnumerator GetPetInfo(Action<Dictionary<int, PetInfo>> onSuccess) {
        ...
        try {
            for (int i = 0; i < basicInfo.Count; i++) {
                ...
                PetInfo info = new PetInfo(basic, featureInfo.Get(basic.baseId), expInfo.Get(basic.id), new PetTalentInfo(), skillInfo.Get(basic.id), uiInfo.Get(basic.id));
