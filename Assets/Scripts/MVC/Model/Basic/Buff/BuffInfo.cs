...
public class BuffInfo
{
    ...
    public Task<Sprite> icon { get => GetIcon(); }
    ...
    public async Task<Sprite> GetIcon() {
        ...
        if (type == BuffType.Feature)
            return await PetUISystem.GetPetIcon(_pet * (_type < 5 ? 1 : -1));
        if (type == BuffType.Emblem)
            return await PetUISystem.GetEmblemIcon(_pet * (_type < 5 ? 1 : -1));

        return await Addressables.LoadAssetAsync<Sprite>("Buffs/" + resId).Task;
    }
