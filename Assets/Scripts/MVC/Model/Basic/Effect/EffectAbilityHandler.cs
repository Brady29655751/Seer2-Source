...
public static class EffectAbilityHandler
{
    ...
    public static bool SetSkill(this Effect effect, BattleState state) {
        ...
        if (type == "id") {
            string setAnger = effect.abilityOptionDict.Get("setAnger", "false");
    ...
    public static bool SetPet(this Effect effect, BattleState state) {
        ...
        if (state == null) {
            ...
            float oldValue = pet.GetPetIdentifier(type);
            float newValue = pet.TryGetPetIdentifier(value, out newValue) ? newValue : Identifier.GetNumIdentifier(value);
