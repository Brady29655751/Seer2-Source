...
public class ActivityInfo 
{
    ...
    public Task<Sprite> activityBackground => GetBackground(resId);
    ...
    public static async Task<Sprite> GetBackground(string resId) {
        if (int.TryParse(resId, out _))
            return await Addressables.LoadAssetAsync<Sprite>("Activities/icon/" + resId).Task;
        else
            return await Addressables.LoadAssetAsync<Sprite>(resId).Task;
    }
